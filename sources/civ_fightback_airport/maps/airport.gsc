...
custom_intro_civilians() {
	flag_init( "bots_shoot_script" );
	flag_init("bots_have_gun");
	...
	level.gunBots = [];
	//---------------Custom Spawner Setup----------------
	zSpawnCoord = 70;
	level.bot_spawnPoints = [];
	level.bot_spawnPointStructs = [];
	level.bot_spawnPoints[0] = (6465,1792,zSpawnCoord);
	level.bot_spawnPoints[1] = (6438,1808,zSpawnCoord);
	level.bot_spawnPoints[2] = (6397,1841,zSpawnCoord);
	level.bot_spawnPoints[3] = (6376,1861,zSpawnCoord);
	level.bot_spawnPoints[4] = (6310,1885,zSpawnCoord);
	level.bot_spawnPoints[5] = (6313,1921,zSpawnCoord);
	level.bot_spawnPoints[6] = (6319,1949,zSpawnCoord);
	level.bot_spawnPoints[7] = (6307,1973,zSpawnCoord);
	level.bot_spawnPoints[8] = (6297,2011,zSpawnCoord);
	level.bot_spawnPoints[9] = (6303,1833,zSpawnCoord);
	level.bot_spawnPoints[10] = (6238,1991,zSpawnCoord);
	level.bot_spawnPoints[11] = (6236,1954,zSpawnCoord);
	level.bot_spawnPoints[12] = (6230,1917,zSpawnCoord);
	level.bot_spawnPoints[13] = (6232,1873,zSpawnCoord);
	wait 21.5;
	for(i = 0; i < 14; i++) {
		doBotSpawn(level.bot_spawnPoints[i], false, i); //spawn the blank bots
	}
	wait 10;
	flag_set("bots_have_gun");
	for(i = 0; i < level.bots.size; i++) {
		level.bots[i] Delete();
		level.bots[i] Destroy();
		doBotSpawn(level.bot_spawnPoints[i], true, i);
	}
	wait 1; //wait until shoot player
	flag_set("bots_shoot_script");

}

doBotSpawn(originPos, hasGun, botNum)
{
	//Get Spawners
	...
	//spawner id list
	//0 = lev (used for bots with guns)
	//1 = viktor
	//2 = kiril
	//3 = makarov
	//4 = annoying people that hide on player (used for blank bots)
	//5 = annoying people that hide on player
	//200 = riot shield swat
	if(hasGun) {
		number = 0;
	}
	else {
	...
	spawners[number].origin = originPos;
	
	//Spawn Bots
	spawners[number].script_forcespawn = true;//force spawner to stalingradspawn ai
	spawners[number].script_playerseek = false;//ai runs to player
	spawners[number].script_pacifist = true;//ai only attacks after you hurt it
	spawners[number].script_ignoreme = true;//ai ignores player
	spawners[number].script_ignoreall = true;
	spawners[number].script_moveoverride = undefined;
	spawners[number].script_patroller = false;
	spawners[number].script_stealth = false;
	spawners[number].script_startrunning = false;
	spawners[number].script_friendname = "";
	spawners[number].script_noteworthy = "none";
	spawners[number].count = 9999;
	bot = spawners[number] stalingradspawn(); //force spawn
	if(!hasGun) {
		bot change_bot_model(false, false, botNum);	//setup random civilian models
	} 
	else {
		bot change_bot_model(false, true, botNum);	//setup previous civilian models
	}
	bot StopAnimScripted();	//stop animations from spawner used
	bot DisconnectPaths();
	bot.team = "axis";
	bot allowedStances("stand"); //force stand
	bot thread forcebotpos(originPos); //keep their goal at their spawn point
	if(hasGun) {
		bot thread botscript(originPos);	//where the scripted events happen
	}
	wait .1;
	if(!hasGun) {
		level.bots = add_to_array(level.bots, bot);
	}
...
change_bot_model(detach, usePreviousModels, botNum) {
	rand = randomInt(5);//0 to 4
	if(detach) {
		self DetachAll();
	}
	if(!usePreviousModels) {
		level.bot_models[botNum] = rand;
		if(rand == 0) {
		...
		if(rand == 2) {
			...
			self attach("head_urban_civ_male_aa", "", true);
			self.headModel = "head_urban_civ_male_aa";
		}
		if(rand == 3) {
			self setModel("body_urban_civ_male_ab");
			self attach("head_urban_civ_male_ab", "", true);
			self.headModel = "head_urban_civ_male_ab";
		}
		if(rand == 4) {
			self setModel("body_urban_civ_male_ac");
			self attach("head_urban_civ_male_ac", "", true);
			self.headModel = "head_urban_civ_male_ac";
		}
	}
	else {
		if(level.bot_models[botNum] == 0) {
			self setModel("body_urban_civ_female_a");
			self attach("head_urban_civ_female_a", "", true);
			self.headModel = "head_urban_civ_female_a";
		}
		if(level.bot_models[botNum] == 1) {
			self setModel("body_urban_civ_female_b");
			self attach("head_urban_civ_female_b", "", true);
			self.headModel = "head_urban_civ_female_b";
		}
		if(level.bot_models[botNum] == 2) {
			self setModel("body_urban_civ_male_aa");
			self attach("head_urban_civ_male_aa", "", true);
			self.headModel = "head_urban_civ_male_aa";
		}
		if(level.bot_models[botNum] == 3) {
			self setModel("body_urban_civ_male_ab");
			self attach("head_urban_civ_male_ab", "", true);
			self.headModel = "head_urban_civ_male_ab";
		}
		if(level.bot_models[botNum] == 4) {
			self setModel("body_urban_civ_male_ac");
			self attach("head_urban_civ_male_ac", "", true);
			self.headModel = "head_urban_civ_male_ac";
		}
...
botscript(pos) {
	//scripted bot events
	//flag_wait("bots_have_gun");
	//self GiveWeapon( "saw_airport" );
	flag_wait("bots_shoot_script");
	self thread botsShoot();
	thread killable_squad();
...
killable_squad() {
	level.makarov.team = "allies";
	level.team[ "m4" ].team = "allies";
	level.team[ "saw" ].team = "allies";
	level.team[ "shotgun" ].team = "allies";

	level.makarov.health = 1;
	level.team[ "m4" ].health = 1;
	level.team[ "saw" ].health = 1;
	level.team[ "shotgun" ].health = 1;
	while(true) {
		level.makarov EnableDeathShield( false );
		level.makarov EnableHealthShield( false );
		level.makarov stop_magic_bullet_shield();

		level.team[ "m4" ] EnableDeathShield( false );
		level.team[ "m4" ] EnableHealthShield( false );
		level.team[ "m4" ] stop_magic_bullet_shield();

		level.team[ "saw" ] EnableDeathShield( false );
		level.team[ "saw" ] EnableHealthShield( false );
		level.team[ "saw" ] stop_magic_bullet_shield();

		level.team[ "shotgun" ] EnableDeathShield( false );
		level.team[ "shotgun" ] EnableHealthShield( false );
...
botsShoot() {
	shootPos = RandomInt(5); //0 to 4
	shootAt = undefined;
	if(shootPos == 0) {//shoot player
		shootAt = level.player.origin;
	}
	if(shootPos == 1){//shoot makarov 
		shootAt = level.makarov.origin;
	}
	if(shootPos == 2){//shoot level.team[ "m4" ] 
		shootAt = level.team[ "m4" ].origin;
	}
	if(shootPos == 3){//shoot level.team[ "saw" ] 
		shootAt = level.team[ "saw" ].origin;
	}
	if(shootPos == 4){//shoot level.team[ "shotgun" ] 
		shootAt = level.team[ "shotgun" ].origin;
	}
	shootInt = RandomFloatRange(0.1, 0.7);
	wait shootInt;
	while(true) {
		self OrientMode("face point", shootAt);
		self shoot();
		wait 0.1;
	}
}
