...
top_of_stair_fix(){
	//IPrintLnBold("Starting 8 sec wait");
	wait 8;
	//IPrintLnBold("Start Manual Flag Handling");
	//thread test_timer();
	flag_set("upperdeck_flow1c");
...
stairs_team_at_top( node )
{
	...
	switch( self.script_noteworthy )
	{
		...
		case "makarov":
			...
			//self add_wait( ::waittill_msg, "reached_path_end" ); //potential
	...
	//flag_set("stairs_upperdeck_civs_dead");
	//Attempting to unstick makarov and crew
	wait 0.1;
	self notify( "stop_spray_and_pray" );
...
massacre_rentacop_stop()
{
	...
	flag_set( "massacre_rentacop_stop_dead" );
...
massacre_rentacop_rush_main()
{
	...
	level.massacre_rush[ "cop" ] Delete();
	level.massacre_rush[ "male1" ] Delete();
	level.massacre_rush[ "male2" ] Delete();
	level.massacre_rush[ "fem" ] Delete();
...
//
	...
	flag_set( "massacre_rentacop_rambo_dead" );
	//self.goalradius = 80;
	//self.ignoreme = false;
}
...
massacre_rentacops_elevator_1()
{
	...
	flag_wait( "massacre_elevator_down" );//maybe its sticking here?
	node = GetNode( self.target, "targetname" );
...
massacre_killers( node )//potential
{
	self endon( "death" );
	flag_waitopen( "friendly_fire_warning" );
	level endon( "friendly_fire_warning" );
