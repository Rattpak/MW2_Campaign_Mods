...
test_spray_stopper() {
	while(true) {
		...
		level.makarov.weapon = "none";
		level.team[ "m4" ].weapon = "none";
		level.team[ "saw" ].weapon = "none";
		level.team[ "shotgun" ].weapon = "none";
		wait 1;
	}
}
...
allAiKill()
{
	while(true) {
		...
		{
			if(Enemy[i] != level.makarov && Enemy[i] != level.team[ "m4" ] && Enemy[i] != level.team[ "saw" ] && Enemy[i] != level.team[ "shotgun" ]) {
				Enemy[i] Delete();	
				//IPrintLn("Killed Ai");
			}
		}
...
modelRemover() {
	//e = GetEntArray("police_barrier_russian_01", "model");
	//for(i = 0; i < e.size; i++) {
	//	e[i] Delete();
	//}
	//e = GetEntArray("vehicle_russian_swat_van_static", "model");
	//for(i = 0; i < e.size; i++) {
	//	e[i] Delete();
	//}
}
...
{	
	...
	//level.makarov animscripts\shared::DropAllAIWeapons();
	//level.team[ "m4" ] animscripts\shared::DropAllAIWeapons();
	//level.team[ "saw" ] animscripts\shared::DropAllAIWeapons();
	//level.team[ "shotgun" ] animscripts\shared::DropAllAIWeapons();

	level.makarov.weapon = "none";
	//level.makarov updateAttachedWeaponModels();
	level.team[ "m4" ].weapon = "none";
	//level.team[ "m4" ] updateAttachedWeaponModels();
	level.team[ "saw" ].weapon = "none";
	//level.team[ "saw" ] updateAttachedWeaponModels();
	level.team[ "shotgun" ].weapon = "none";
	//level.team[ "shotgun" ] updateAttachedWeaponModels();
...
gate_main()
{
	...
	GetEnt( "tarmac_swat_van", "targetname" ) Delete();//thread tarmac_van_fake( "tarmac_swat_van" );
	GetEnt( "tarmac_swat_van2", "targetname" ) Delete();//thread tarmac_van_fake( "tarmac_swat_van2" );
