...
/#
	...
	//lock check
#/
	if(message != "upperdeck_save" && message != "upperdeck_flow2" && message != "gate_player_ready"){
		//IPrintLn("^5flag set: ^7" + message);
	}
...
{	
	//IPrintLn("^2flag_wait ^7" + msg);
	if(isdefined(level.flag_wait_active)) {
		level.flag_wait_active = add_to_array(level.flag_wait_active, msg);
	}
	else {
		level.flag_wait_active = [];
	}
	other = undefined;
	while ( !flag( msg ) )
	{
	...
	//IPrintLn("^1flag_wait complete ^7" + msg);
	level.flag_wait_active = array_remove(level.flag_wait_active, msg);
	if ( isdefined( other ) )
		return other;
}

get_active_flag_wait() {
	return level.flag_wait_active;
}
