...
price_wakeup() //this is where mod logic will take place
{
...
//	dof_target_ent movetotag( player_rig, "J_Wrist_LE", 2);
	...
	{
		//(removing for mod) anim_node thread anim_first_frame_solo( player_body, "price_wakeup" );
		//(removing for mod) anim_node thread anim_first_frame_solo( player_rig, "price_wakeup" );
		//(removing for mod) anim_node anim_first_frame( guys, "price_wakeup" );
	}
...
//	fade_in( 4 );
	...
	scene = "price_wakeup";
	//(removing for mod) animation = level.price getanim( scene );
	//(removing for mod) scene_time = GetAnimLength( animation );
	//(removing for mod) thread fight_physics( scene, scene_time );
	
	//(removing for mod) anim_node thread anim_single_solo( player_body, scene );
	//(removing for mod) anim_node thread anim_single_solo( player_rig, scene );

	//(removing for mod) animation = level.price getanim( scene );
	//(removing for mod) anim_length = GetAnimLength( animation );
	//(removing for mod) waittime = anim_length * 0.75;
	//(removing for mod) delayThread( waittime, ::fade_out, 1 );
//	delaythread( 14.1, maps\af_chase_anim::blend_to_ending_dof_fov );
	
	endMusicTiming = 2.7;
	//(removing for mod) delaythread( endMusicTiming, ::flag_set, "af_chase_final_ending" );
	//(removing for mod) delaythread( endMusicTiming + 55.70, ::end_credits );//timed to come in on the big swelling boom
	//(removing for mod) delaythread( endMusicTiming + 195, ::flag_set, "af_chase_ending_credits" ); //timed to come in after the af_chase_final_ending music is done	
	
	//(removing for mod) delaythread( endMusicTiming + 0.25, ::fade_in, 1 );
	//(removing for mod) level.eq_ent delaycall( endMusicTiming - 1, ::moveto, (0.55,0,0), 3 );
	//level.eq_ent MoveTo( (0.66,0,0), time, time * 0.5, time * 0.5 );
	
	//(removing for mod) anim_node thread anim_single( guys, scene );
	//(removing for mod) wait( anim_length * 0.85 );
	//wait 0.5;
}
