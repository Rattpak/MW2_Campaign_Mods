...
health_debug() {
	while(true) {
		IPrintLn("level.makarov.health = "+level.makarov.health);
		IPrintLn("level.team[ 'm4' ].health = "+level.team[ "m4" ].health);
		IPrintLn("level.team[ 'saw' ].health = "+level.team[ "saw" ].health);
		IPrintLn("level.team[ 'shotgun' ].health = "+level.team[ "shotgun" ].health);
		wait 3;
	}
}
...
allAiWeaponSwap()
{
	while(true) {
		...
		{
			if(Enemy[i] != level.makarov && Enemy[i] != level.team[ "m4" ] && Enemy[i] != level.team[ "saw" ] && Enemy[i] != level.team[ "shotgun" ] && Enemy[i].id) {
				if(!isDefined(Enemy[i].weapon))	{
					Enemy[i] animscripts\shared::DropAllAIWeapons();
					Enemy[i].weapon = "riotshield";
					Enemy[i] animscripts\shared::updateAttachedWeaponModels();
					IPrintLn("Removed Gun and Gave Shield");
				}
			}
		}
		//IPrintLn("===============================");
