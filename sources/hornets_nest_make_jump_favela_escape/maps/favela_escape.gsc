...
main()
{
	...
	level.player thread manual_jump_watch();
}
...
manual_jump_watch() {
	...
	while(true) {
		if (self FragButtonPressed()) {
			//self record_jump_origins();
			self manual_jump_script();
		}
		wait 0.1;
	}
}
...
record_jump_origins() {
	...
	IPrintLnBold("1");
	wait 1;
	IPrintLnBold("^1Recording Positions");
	PrintLn("==============================================");
	while(self FragButtonPressed() == false) {
		PrintLn("self teleport_to_origin("+self.origin+", (0, 180, 0));");
		wait 0.05;
	}
	PrintLn("==============================================");
	IPrintLnBold("^1Done");
...
manual_jump_script() {
	self teleport_to_origin((-6332.58, -955.975, 1069.88), (0, 180, 0));
}
