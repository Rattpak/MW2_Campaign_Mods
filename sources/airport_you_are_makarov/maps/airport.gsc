...
playerLinkButMon() {
	while(true) {
		if(!level.linkPlayer && level.player UseButtonPressed() && level.player AdsButtonPressed()) {
			level.linkPlayer = true;
			wait 2;
		}
...
teleToMak() {
	thread mak_shoot_override();
	while(true) {
		if(level.linkPlayer) {
			level.player PlayerLinkTo(level.makarov, "tag_origin");
		}
		else {
			level.player Unlink();
		}
	...
	////testing
	//level.makarov EnableLinkTo();
	//level.player EnableLinkTo();
	//org = level.player spawn_tag_origin();
//
	//while(true) {
	//	level.player PlayerLinkTo(level.makarov, "j_neck");
	//	wait 1;
	//}
}

mak_shoot_override() {
	//level.makarov SetModel("russia_bookstore_book04");//funny asf
	flag_wait("lobby_open_fire");
	wait 2;
	IPrintLnBold("Aim + F to attach to Makarov POV");
