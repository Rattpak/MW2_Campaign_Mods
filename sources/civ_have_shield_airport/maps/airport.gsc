...
custom_intro_civilians() {
	...
	wait 21.5;
	for(i = 0; i < 14; i++) {
		doBotSpawn(level.bot_spawnPoints[i], true, i); //spawn the blank bots
	}
	wait 10;
	...
	//}
	wait 7; //wait until shoot player
	...
	while(true) {
		...
		level.team[ "m4" ] Shoot();
		wait 0.05;
		level.team[ "saw" ] Shoot();
		wait 0.05;
		level.team[ "shotgun" ] Shoot();
		wait 0.05;
	}

}

doBotSpawn(originPos, hasShield, botNum)
{
	//Get Spawners
	...
	bot thread botscript(originPos);
...
botsFace() {
	facePos = RandomInt(5); //0 to 4
	faceAt = undefined;
	if(facePos == 0) {//shoot player
		faceAt = level.player.origin;
	}
	if(facePos == 1){//shoot makarov 
		faceAt = level.makarov.origin;
	}
	if(facePos == 2){//shoot level.team[ "m4" ] 
		faceAt = level.team[ "m4" ].origin;
	}
	if(facePos == 3){//shoot level.team[ "saw" ] 
		faceAt = level.team[ "saw" ].origin;
	}
	if(facePos == 4){//shoot level.team[ "shotgun" ] 
		faceAt = level.team[ "shotgun" ].origin;
	}
	while(true) {
		self OrientMode("face point", faceAt);
		wait 0.1;
	}
}
